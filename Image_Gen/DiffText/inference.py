...
def main():
    ...
    #  tokenizer and models(text_encoder,vae,unet).
    ...
    # print("Unet Configuration: ",unet.config)
    ...
    # noise = torch.randn((sample_num, 17, 64, 64)).to("cuda")  # (b, 17, 64, 64)

    input = noise # (b, 4, 64, 64)

    captions = [args.prompt] * sample_num
    captions_nocond = [""] * sample_num #empty prompt


    print(f'{colored("[√]", "green")} Prompt is loaded: {args.prompt}.')
    
    # encode text promtext_segmenterpts
    inputs = tokenizer(
        captions, max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
    ).input_ids # (b, 77)
    encoder_hidden_states = text_encoder(inputs)[0].cuda() # (b, 77, 768) encodes token to embedding(for 1 token 768 sized embedding)
    print(f'{colored("[√]", "green")} encoder_hidden_states: {encoder_hidden_states.shape}.')

    # similar as above with the condition of no prompt
    ...
    segmenter.load_state_dict(torch.load(args.character_segmenter_path))  #loading the trained unet model
    segmenter.eval()
    ...
    if args.mode == 'text-to-image':    
        ...
        segmentation_mask = torch.nn.functional.interpolate(segmentation_mask.unsqueeze(0).unsqueeze(0).float(), size=(256, 256), mode='nearest') #(1,1,256,256)
        segmentation_mask = segmentation_mask.squeeze(1).repeat(sample_num, 1, 1).long().to('cuda') # (b, 256, 256)
    ...
    for t in tqdm(scheduler.timesteps):
        with torch.no_grad():
            print(input.shape)
            noise_pred_cond = unet(sample=input, timestep=t, encoder_hidden_states=encoder_hidden_states,added_cond_kwargs={
                segmentation_mask:segmentation_mask, 
                feature_mask:feature_mask, 
                masked_feature:masked_feature
            }).sample # b, 4, 64, 64
            noise_pred_uncond = unet(sample=input, timestep=t, encoder_hidden_states=encoder_hidden_states_nocond,added_cond_kwargs={
                segmentation_mask:segmentation_mask, 
                feature_mask:feature_mask, 
                masked_feature:masked_feature
            }).sample # b, 4, 64, 64
            ...
            print("the shape of noise_pred_cond:",noise_pred_cond.shape)
            print("the shape of noise_pred_uncond:",noise_pred_uncond.shape)

            noisy_residual = noise_pred_uncond + args.classifier_free_scale * (noise_pred_cond - noise_pred_uncond) # b, 17, 64, 64     
